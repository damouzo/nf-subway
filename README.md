...
- 🌈 **Color-coded status** - Pending (⏳), Running (⚡), Complete (✅), Failed (❌), Cached (💾)
- 📊 **Progress tracking** - Duration display for completed processes
- 🔄 **Multiple modes** - Pipe mode, log file monitoring
...
```bash
git clone https://github.com/yourusername/nf-subway.git
cd nf-subway
pip install -e .
```
